...
beforeAll(help.getTester);
afterAll(help.closeTester);
    ...
    describe(vendor.name, () => {
        ...
        describe('Issue Credential HTTP API', () => {
          ...
          vendor.issuers.forEach(issuer => {
            describe(`${issuer.name} ${issuer.endpoint}`, () => {
              beforeEach(() => {
                const clonedVendorCredentials = help.cloneObj(
                  vendor.credentials
                );
                credentials = help.annotateWithUniqueId(
                  clonedVendorCredentials
                );
              });
              ...
              describe('3.1 Can issue with all supplied options', () => {
                it(`test each option for ${issuer.name}`, async () => {
                  await Promise.all(
                    issuer.options.map(async issuerOptions => {
                      const body = {
                        credential: credentials[0],
                      ...
                      if (
                        // eslint-disable-next-line max-len
                        issuerVms.indexOf(res.body.proof.verificationMethod) ===
                        -1
                      ) {
                        issuerVms.push(res.body.proof.verificationMethod);
                      }
                    })
                  );
                });
          ...
          describe(`3. The Issuer's Issue Credential HTTP API MUST support the issuance of credentials with at least 2 different DID methods as the "issuer" on a Verifiable Credential.`, () => {
            ...
            it('meets criteria after all issuers have been tested', async () => {
              expect(issuerVms.length).toBeGreaterThanOrEqual(2);
            });
          });
        });
      });
      ...
      describe('Verifier', () => {
        describe('Verify Credential HTTP API', () => {
          ...
          beforeEach(() => {
            verifiableCredentials = help.cloneObj(
              vendor.verifiable_credentials
            );
          });
          // eslint-disable-next-line max-len
            ...
            it('should pass', async () => {
              ...
              await Promise.all(
                vendor.verifiable_credentials.map(async vc => {
                  ...
                  expect(res.status).toBe(200);
                  if (uniqueIssuers.indexOf(vc.issuer) === -1) {
                    uniqueIssuers.push(vc.issuer);
                  }
                })
              );
              expect(uniqueIssuers.length).toBeGreaterThanOrEqual(2);
            });
          });
        ...
        describe('Verify Presentation HTTP API', () => {
          ...
          beforeEach(() => {
            verifiablePresentations = help.cloneObj(
              vendor.verifiable_presentations
            );
          });
          // eslint-disable-next-line max-len
          describe(`1. The Verifier's Verify Presentation HTTP API MUST verify a Verifiable Presentation where the credential's issuer, presentation's holder and credential's subject are different.`, () => {
            it('should pass', async () => {
              ...
              const testVps = verifiablePresentations.filter(vp => {
                return (
